...
df1 = pd.read_csv(os.path.join(dir, 'velocity.csv'),
                  delimiter=';', skiprows=0, low_memory=False)

df2 = pd.read_csv(os.path.join(dir, 'carlaBridge_velocity.csv'),
                  delimiter=';', skiprows=0, low_memory=False)

data1 = df1['prom_velocity'].to_numpy()
data2 = df2['Velocity'].to_numpy()

p1.plot(df1['Time'], data1, pen="r")
p1.plot(df2['Time'], data2, pen="g")
axis = DateAxisItem()
...
def mouseMoved(evt):
    ...
    if p1.sceneBoundingRect().contains(pos):
        ...
        nearest2 = find_nearest(df2['Time'], xpoint)
        index2 = df2.index[df2['Time'] == nearest2].tolist()[0]
        ...
        label.setText(
            ...
            "<span style='color: green'>Carla=%0.1f</span>" % (
                datetime_, data1[index1], data2[index2]))
